...
def loss_calculation(pred_r, pred_t, choose, target_rt,target_trans, idx, points, w,
                                refine, choose_patchs,num_point_mesh,
                                sym_list, rot_list,ref_list,
                                nosym_list,target,model_points,normals,model_info,model_axis):
    ...
    if idx[0].item() in rot_list:
        ...
        if idx[0].item() in nosym_list:
            ...
            # target_pcd = Pointclouds(points=[target.view(-1, 3)])
            # target_normals = target_pcd.estimate_normals(neighborhood_size=30, disambiguate_directions=True,
            #                                              assign_to_self= True)
            # pred_pcd = Pointclouds(points=[pred.view(-1, 3)])
            # pred_normals = pred_pcd.estimate_normals(neighborhood_size=30, disambiguate_directions=True,
            #                                          assign_to_self=True)
            # cf_loss, norm_loss = chamfer_distance(target_pcd, pred_pcd)
            loss = dis_model
        ...
        if idx[0].item() in ref_list:
            ...
            # model_axis_p = model_axis.view(bs, 1, 3)
            # target_axis_v = torch.add(torch.bmm(model_axis_p, target_r), target_t) - target_t
            # pred_axis_v = torch.add(torch.bmm(model_axis_p, base), pred_t) - pred_t
            # theta = torch.acos(torch.bmm(target_axis_v, pred_axis_v.permute(0, 2, 1)) / \
            #                    (torch.norm(pred_axis_v, dim=2).view(-1) + 0.0001)) / math.pi
            sym_trans_ls = model_info["symmetries_discrete"]
            sym_dis_sum = 0
            sym_dis_ls = []
            # sym_dis_ls.append(dis_model)
...
class Loss(_Loss):
    ...
    def forward(self, pred_r, pred_t, pred_c, target_rt, target_trans,idx, points, w,
                refine, choose_patchs, target, model_pt, normals,model_info,model_axis):

        return loss_calculation(pred_r, pred_t, pred_c, target_rt,target_trans, idx, points, w,
                                refine, choose_patchs,self.num_pt_mesh,
                                self.sym_list, self.rot_list,self.ref_list,
                                self.nosym_list,target,model_pt,normals,model_info,model_axis)
