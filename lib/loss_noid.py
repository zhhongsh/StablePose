...
# (pred_r, pred_t, choose_r, target_rt, points,target,model_pt,target_mode,model_info
def loss_calculation(pred_r, pred_t, pred_a,choose, target_rt, points, target, model_points, target_mode, target_s):
    ...
    if target_mode==1:
        pred_axis = F.normalize(pred_a,p=2,dim=1).view(bs,-1)
        ...
        for i in range(target_trans.shape[0]):
            for j in range(base.shape[0]):
                ...
                target_pt_ = torch.bmm(sym_trans, model_points_).permute(0, 2, 1)[:, :, :3]
...
class Loss(_Loss):
    ...
    def forward(self, pred_r, pred_t,pred_a, choose_r, target_rt,points, target,model_pt,target_mode, model_info):

        return loss_calculation(pred_r, pred_t, pred_a,choose_r, target_rt, points,target,model_pt,target_mode,model_info)
