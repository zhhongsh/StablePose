...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, root, noise_trans, refine):
        if mode == 'train':
            ...
            self.mode = 'test'
            self.path = proj_dir + 'datasets/linemod/dataset_config/lmo_test_list.txt'
        self.num_pt = num_pt
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'test':
            ...
            label = np.array(Image.open(
                '{0}/{1}/{2}/{3}_{4}.png'.format('/home/dell/yifeis/pose/pose_est_tless_3d/tools/lmo/mask_detection_pix2pose',
                                                 data_dir, 'mask_visib', data_num, str(idx).zfill(6))))
