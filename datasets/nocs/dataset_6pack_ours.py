...
class Dataset(data.Dataset):
    ...
    def get_frame(self, choose_frame, choose_obj, syn_or_real):
        ...
        choose = np.array([i for i in range(pts_ori_clear.shape[0])])
        # choose = mask_label.flatten().nonzero()[0]
        # choose_patch = mask.flatten().nonzero()[0]
        if len(choose) == 0:
            return 0
        ...
        # np.savetxt('/home/lthpc/yifeis/pose/pose_est_tless_3d/datasets/nocs/test/pts_ori.txt', clouds)
        ...
        mesh_trans = np.dot(mesh[0],rotation.T)+target_t
        # np.savetxt('/home/lthpc/yifeis/pose/pose_est_tless_3d/datasets/nocs/test/mesh.txt', mesh[0])
        # np.savetxt('/home/lthpc/yifeis/pose/pose_est_tless_3d/datasets/nocs/test/mesh_trans.txt', mesh_trans)
        ...
        mesh = mesh.detach().data.numpy()
        return [cloud, # observed point cloud
                pts,  # clouds trans to mesh coordinate
                mesh,  # mesh from the normalized model
                target_r, target_t,
                mesh_trans] # mesh trans to camera(clouds) coordinate
    ...
    def __getitem__(self, index):
        ...
        cloud, trans_clouds, mesh,target_r, target_t, mesh_trans = self.get_frame(choose_frame[0], choose_obj, syn_or_real)
